...
task * scheduler_n() {
    if (ready_queue != NULL)            //If the ready queue isn't empty, we have tasks ready to be returned from the scheduler
    {
        ...
        {
            task *cursor = ready_queue; //Pekare så vi inte flyttar head-pekaren till ready_queue
            task *SJcursor = ready_queue; //pekare för att spara vilket shortest job first är

            int shortestJob = 1000; //Stor siffra så inget kommer råka vara större(dummy-värde)
            while (cursor != NULL) //Stega igenom hela ready_queue
            {
                if (cursor->quantum < shortestJob) //Om jobbet är det kortaste hitills, spara det jobbet i en pekare
                {
            ...
            while (ready_queue != SJcursor) //Kasta alla job som inte är det kortaste till slutet av listan tills rätt jobb finns.
            {
        ...
        if (sched_type == sched_MQ)        //Here is where you implement your MQ scheduling algorithm,
        {
            ...
            task *cursor = ready_queue; //temporär pekare så vi inte ändrar i ready_queue

            while (cursor != NULL) { //Stegar igenom hela kön
                if (cursor->priority == HIGH_PRIO && HP != 1) //Stöter vi på ett job med prioritet HÖG, så sätt en pekare dit, och HP till 1. När den första högprion har stötts på kommer den inte gå in i if-satsen igen
                {
                    prioritycursor = cursor;
                    HP = 1;

                } else if (cursor->priority >= MEDIUM_PRIO && cursor->priority < LOW_PRIO && cursor->priority % MEDIUM_PRIO != 0 && HP != 1 && MPMOD != 1){ //Körs om den är Medium priority, och om den redan kört ett tidskvanta, vilket prioriteras högre än inga tidskvanta, och om den hittat 1 går den inte in igen
                    prioritycursor = cursor;
                    MPMOD = 1;
                }
                else if (cursor->priority >= MEDIUM_PRIO && cursor->priority < LOW_PRIO && cursor->priority % MEDIUM_PRIO == 0 && HP != 1 && MPMOD != 1 && MP != 1) //Precis som funktionen ovan, men körs bara om den inte kört någon tidskvanta
                    {
                        prioritycursor = cursor;
                        MP = 1;
                } else if (cursor->priority >= LOW_PRIO && cursor->priority % 4 != 0 && MP != 1 && HP != 1 && LPMOD != 1){ //kollar att prion är Låg, och körs bara om den redan kört en tidskvatna, som prioriteras över inga tidskvanta, och körs inte om den hittat ett passade jobb                {
                    prioritycursor = cursor;
                    LPMOD = 1;

                } else if (cursor->priority >= LOW_PRIO && cursor->priority % 4 == 0 && MP != 1 && HP != 1 && LPMOD != 1 && LP!=1) //som funktionen ovan, fast bara om den inte kört någon todskvanta sen den kom till låg prio.
                {
                    prioritycursor = cursor;
                    LP = 1;
            ...
            if (HP == 1 || MP == 1 || LP == 1||LPMOD == 1||MPMOD==1) {
                ...
                {
                    ready_queue = first_to_last(ready_queue); //Sätter högprio jobbet först som hittades tidigare
                }
                prioritycursor->priority = prioritycursor->priority + 1; //ökar priovärdet så den kan får en lägre prioritet
                return ready_queue;
